...
/// Parse a `liveness.edn` sidecar — a flat EDN map of `:keyword number` pairs, e.g.
/// `{:d1-rate 0.5, :d7-rate 0.5, :median-fork-depth 2, :remix-cta-clicks 3}` — into a
/// `LivenessMeta`. This is the consumer side of `isekai.bake-liveness` (the ls-h1 ledger →
/// build-gate join, Co-Scientist iter-12/16): the SAME numbers the live game computes feed the
/// quality axis. Tolerant of `;;` comment lines and commas; returns `None` if there is no map.
pub fn parse_liveness_edn(s: &str) -> Option<LivenessMeta> {
    let body: String = s
        .lines()
    ...
    let toks: Vec<&str> = inner
        ...
        .filter(|t| !t.is_empty())
        .collect();
    let mut m = LivenessMeta::default();
    let mut i = 0;
    while i + 1 < toks.len() {
        let (key, val) = (toks[i], toks[i + 1]);
        match key {
            ":d1-rate" => m.d1_rate = val.parse().unwrap_or(0.0),
            ":d7-rate" => m.d7_rate = val.parse().unwrap_or(0.0),
            ":median-fork-depth" => m.median_fork_depth = val.parse().unwrap_or(0.0),
            ":remix-cta-clicks" => m.remix_cta_clicks = val.parse().unwrap_or(0),
            _ => {}
        }
        i += 2;
    }
...
impl GameSceneMeta {
    /// Load the behavioral liveness from a `liveness.edn` sidecar string (the artifact
    /// `isekai.bake-liveness` produces). `gftd build` calls this before `evaluate` so the
    /// Liveness axis grades against the live D1/D7/fork-depth. Unparseable ⇒ `None` (the axis
    /// then scores on static presence only — honest, never faked).
    pub fn with_liveness_edn(mut self, edn: &str) -> Self {
        self.liveness = parse_liveness_edn(edn);
        self
    }
}
...
mod tests {
    ...
    #[test]
    fn parses_liveness_edn_sidecar() {
        // exactly what isekai.bake-liveness emits (comments + commas tolerated).
        let edn = ";; GENERATED by isekai.bake-liveness\n;; do not hand-edit\n\
                   {:d1-rate 0.5, :d7-rate 0.5, :median-fork-depth 2, :remix-cta-clicks 3}\n";
        let l = parse_liveness_edn(edn).expect("parses");
        assert_eq!(l.d1_rate, 0.5);
        assert_eq!(l.d7_rate, 0.5);
        assert_eq!(l.median_fork_depth, 2.0);
        assert_eq!(l.remix_cta_clicks, 3);
    }

    #[test]
    fn liveness_edn_feeds_the_axis() {
        // the sidecar drives the behavioral half of the Liveness axis (above the static 10).
        let meta = sabiotoshi_meta()
            .with_liveness_edn("{:d1-rate 0.4 :d7-rate 0.2 :median-fork-depth 3 :remix-cta-clicks 120}");
        let report = evaluate(&meta);
        let liveness = report.axes.iter().find(|a| a.name == "Liveness").unwrap();
        assert!(liveness.score > 10.0, "static 10 + behavioral trend, got {}", liveness.score);
    }

    #[test]
    fn malformed_liveness_edn_is_none() {
        assert!(parse_liveness_edn("not edn at all").is_none());
        assert!(parse_liveness_edn("").is_none());
    }
