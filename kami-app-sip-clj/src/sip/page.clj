(ns sip.page
  "Graphic-novel PAGE composition — the manga flow as Clojure. Ported from
  mangaka.gftd.ai's `manga-layouts.ts` GRAPHIC_NOVEL_TEMPLATES (left-to-right
  reading, per the story-bible style guide), this places the rendered panel
  images into a B5 page, draws panel frames + gutters, and overlays dialogue as
  speech bubbles and narration as caption boxes — turning isolated panels into a
  readable graphic-novel page.

  JVM/Java2D authoring (headless); no Canvas-2D, no GPU — this is page DTP, not
  the game engine's wgpu render path."
...
;; full-bleed splashes. We derive that from each panel's gh:size + the page
;; layout, so the composition follows the artist's ネーム, not a generic grid.
...
(defn compose-page!
  "Compose one storyboard `page` (from sip.storyboard/pages) into a B5 PNG at
  `out`. `img-of` maps a panel-id → image File (or nil → placeholder)."
  [page img-of out]
