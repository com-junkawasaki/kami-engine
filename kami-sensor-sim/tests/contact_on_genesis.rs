...
fn contact_at(
    ...
    world.articulation_mut(h).unwrap().set_joint_positions(&[x, 0.0]);
...
fn cart_contact_sensor_detects_obstacle_and_penetration_grows() {
    ...
    scene.add(Primitive::Sphere { center: Vec3::new(1.0, 0.0, 0.0), radius: 0.3 });
    ...
    assert!((far.closest_distance - 0.4).abs() < 1e-3, "clearance {} != 0.4", far.closest_distance);
    ...
    assert!((touch.penetration_depth - 0.1).abs() < 1e-3, "penetration {} != 0.1", touch.penetration_depth);
    ...
    assert!(deep.penetration_depth > touch.penetration_depth, "penetration did not grow: {} -> {}", touch.penetration_depth, deep.penetration_depth);
    assert!((deep.penetration_depth - 0.3).abs() < 1e-3, "penetration {} != 0.3", deep.penetration_depth);
    // Contact normal is finite and unit-ish.
    assert!((deep.contact_normal.length() - 1.0).abs() < 1e-3, "normal not unit: {:?}", deep.contact_normal);
}
