...
fn cart_mounted_lidar_range_to_obstacle_shrinks_as_it_approaches() {
    ...
    scene.add(Primitive::Sphere { center: Vec3::new(D, 0.0, 0.0), radius: R });
    ...
    assert!((r1 - (D - cart_x - R)).abs() < 0.25, "range {r1} != geometric {}", D - cart_x - R);
}
...
fn empty_scene_yields_no_obstacle_returns() {
    ...
    let mut lidar = Lidar::new("lidar", "/World/cart/lidar", LidarIntrinsics::vlp16());
    assert!(sense_obstacle(&world, h, &mut lidar, &scene).is_none(), "empty scene should miss");
}
