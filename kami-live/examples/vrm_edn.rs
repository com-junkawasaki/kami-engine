//! VRM dance fully DATA-DRIVEN by clj/edn `:dance/avatar` (vrm path / spring-bones
//! toggle / scale, read from scene.edn). Change the EDN → change the render. +
...
async fn run() {
    ...
    let dist = height * 1.5 / avatar_scale.max(0.1);
    ...
    for frame in 0..32 {
        ...
        // SPRING BONES (gated by EDN :spring-bones): simulate hair/gear, re-run FK.
        if spring_enabled {
    ...
    println!("wrote seed_edn.gif + seedtex_*.png — Seed-san: (clj/edn-driven avatar)");
}
